...
public class Searcher : ISearcher
{
    ...
    public Searcher(CachePolicy cachePolicy = CachePolicy.Content, string? dbPath = null)
    {
        ...
        {
            dbPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "data", "ip2region.xdb");
            if (!File.Exists(dbPath)) throw new FileNotFoundException($"文件不存在，请前往https://github.com/lionsoul2014/ip2region/tree/master/data下载xdb文件：{dbPath}");
        }
